...
import { Sparkles, Trash, Loader2, AlertTriangle, RotateCcw, Smartphone, Monitor } from 'lucide-react';
import { findBestMediaBatch, findBestMixedMediaBatch, type MediaItem } from '@/components/video/mediaLibrary';
...
export function Phase1Ideation() {
    ...
    useEffect(() => {
        ...
        fetch('/api/admin/video/headlines')
            .then(res => res.json())
            .then(data => {
                setRssHeadlines(data.headlines || []);
                setRssSources(data.sources || []);
                setRssFailedFeeds(data.failedFeeds || []);
            })
            .catch(() => { })
    ...
    const parseScript = (text: string, videoFormat: string = '16:9') => {
        ...
        const calcDuration = (speech: string): number => {
            return SyncEngine.estimateDuration(speech);
        };

        const extractHeadline = (block: string): string | undefined => {
            const match = /\[Headline:\s*(.*?)\]/i.exec(block);
            return match ? match[1].trim() : undefined;
        };

        const extractSubHeadline = (block: string): string | undefined => {
            const match = /\[Subheadline:\s*(.*?)\]/i.exec(block);
            return match ? match[1].trim() : undefined;
        };

        const extractVisual = (block: string): string => {
            const match = /\[Visual:\s*(.*?)\]/i.exec(block);
            return match ? match[1].trim() : "Dynamic technology news visualization";
        };

        const extractTransition = (block: string): TransitionType => {
            const match = /\[Transition:\s*(.*?)\]/i.exec(block);
            const t = match ? match[1].toLowerCase().trim() : 'crossfade';
            return validTransitions.includes(t as TransitionType) ? (t as TransitionType) : 'crossfade';
        };

        const extractPexelsQuery = (block: string): string | undefined => {
            const match = /\[PexelsQuery:\s*(.*?)\]/i.exec(block);
            return match ? match[1].trim() : undefined;
        };
        ...
        if (parsedScenes.length < 1) {
            ...
            if (paragraphs.length >= 2) {
                paragraphs.forEach((para) => {
                    const visual = extractVisual(para);
                    const transition = extractTransition(para);
                    const speech = cleanSpeech(para);

                    if (speech && speech.length > 10) {
                        parsedScenes.push({ speech, visual, transition, duration: calcDuration(speech) });
                    }
                });
            } else {
                ...
                for (let i = 0; i < sentences.length; i += SENTENCES_PER_SCENE) {
                    ...
                    parsedScenes.push({
                        speech: chunk,
                        visual: "Technology news background, cinematic lighting",
                        transition: i === 0 ? 'none' : 'crossfade',
                        duration: calcDuration(chunk),
                    });
                }
            }
        }
        ...
        if (finalScenes.length === 0) {
            ...
            if (speech.length > 0) {
                return [{
                    speech,
                    visual: "Dynamic News Visualization",
        ...
        if (totalDuration > MAX_TOTAL_SECONDS) {
            ...
            finalScenes.forEach(s => {
                s.duration = Math.max(3, Math.round((s.duration || 10) * scale));
            });
        }
    ...
    const handleGenerateScript = async () => {
        ...
        try {
            ...
            if (!response.ok) {
                setError(data.details || data.error || "Failed to generate script");
                return;
            }

            if (data.failedFeeds && data.failedFeeds.length > 0) {
                ...
                const scenesWithMedia = await Promise.all(parsed.map(async (s: any, i: number) => {
                    ...
                    try {
                        const res = await fetch('/api/admin/video/assets', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json' },
                            body: JSON.stringify({ query: searchQuery, type: 'both', count: 4 }),
                        });
                        if (res.ok) {
                    ...
                    return {
                        ...s,
                        assetUrl: batch[0]?.url || undefined,
            ...
            logger.error("Error generating script: ", err);
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-5xl mx-auto">
            <div className="glass-card p-6 space-y-6 relative overflow-hidden group">
                ...
                <div className="flex items-center gap-5">
                    ...
                    {/* Format Toggle Integrated into the card */}
                    <div className="flex gap-1 bg-zinc-100/80 p-1.5 rounded-2xl border border-zinc-200/50 backdrop-blur-2xl">
                        <button
                            onClick={() => setFormat("9:16")}
                            className={cn(
                                "flex items-center gap-2 h-9 px-4 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all", 
                                format === "9:16" 
                                    ? "bg-studio-accent text-white shadow-[0_0_15px_rgba(0,122,255,0.4)]" 
                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between px-2">
                        <h3 className="text-[11px] font-bold text-zinc-400 uppercase tracking-[0.2em] px-2 flex items-center gap-2">
                            <div className="w-1.5 h-1.5 rounded-full bg-studio-accent shadow-[0_0_8px_rgba(0,122,255,0.4)]" />
                    ...
                    <div className="max-h-[180px] overflow-y-auto rounded-2xl border border-zinc-200/50 bg-white/40 backdrop-blur-md divide-y divide-zinc-100/50 custom-scrollbar">
                        {rssLoading ? (
                            <div className="flex flex-col items-center justify-center py-8 gap-3 text-zinc-600">
                                <Loader2 className="w-5 h-5 animate-spin text-studio-accent" />
                                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">Fetching World Intelligence...</span>
                            </div>
                        ) : rssHeadlines.length === 0 ? (
                            <div className="flex items-center justify-center py-8 text-zinc-500 text-[10px] font-bold uppercase tracking-widest">
                                No headlines available. Check your internet connection.
                            </div>
                        ) : (
                            rssHeadlines.slice(0, 25).map((h, i) => (
                                <div key={i} className="flex items-start gap-4 px-5 py-4 hover:bg-white/60 transition-all cursor-default group">
                                    <span className={cn(
                                        "text-[9px] font-bold uppercase tracking-wider px-2.5 py-1 rounded-full shrink-0 mt-0.5 border",
                                        h.source === 'TechCrunch' ? 'bg-green-500/10 text-green-400 border-green-500/20' :
                                            h.source === 'The Verge' ? 'bg-studio-accent/10 text-studio-accent border-studio-accent/20' :
                                                h.source === 'Ars Technica' ? 'bg-orange-500/10 text-orange-400 border-orange-500/20' :
                                                    h.source === 'Hacker News' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
                                                        'bg-studio-surface text-zinc-400 border-studio-border'
                                    )}>
                                        {h.source.slice(0, 5)}
                                    </span>
                                    ...
                                    {h.date && (
                                        <span className="text-[10px] font-mono text-zinc-700 shrink-0 mt-0.5">
                                            {Math.round((Date.now() - new Date(h.date).getTime()) / 3600000)}h
                                        </span>
                                    )}
                                </div>
                            ))
                ...
                <div className="space-y-4">
                    ...
                    <div className="relative group">
                        <input
                            className="w-full bg-zinc-50 border border-black/10 rounded-2xl px-6 py-5 text-sm font-medium text-zinc-900 placeholder:text-zinc-400 focus:ring-4 focus:ring-studio-accent/10 focus:border-studio-accent/20 transition-all outline-none"
                            placeholder="e.g. AI breakthroughs, cybersecurity, startups..."
                            value={newsFocus}
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex bg-zinc-100/80 p-1.5 rounded-2xl border border-zinc-200/50">
                            <button
                                ...
                                className={cn(
                                    "flex-1 py-4 rounded-xl transition-all text-[10px] font-bold uppercase tracking-[0.2em]", 
                                        scriptLang === "en" 
                                            ? "bg-studio-accent text-white shadow-xl shadow-studio-accent/30" 
                                            : "text-zinc-500 hover:text-zinc-900"
                                )}
                            >
                                English
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex bg-zinc-100/80 p-1.5 rounded-2xl border border-zinc-200/50">
                            {[
                                ...
                                >
                                    {opt.label}<span className="text-[8px] ml-0.5 opacity-60">min</span>
                                </button>
                            ))}
                        </div>
                    </div>
                    <div className="flex flex-col justify-end gap-4">
                        {error && (
                            <div className="bg-red-500/5 border border-red-500/20 text-red-500 text-[10px] font-bold uppercase tracking-wider p-4 rounded-[1.25rem] flex items-start gap-3 animate-in fade-in slide-in-from-bottom-2">
                                ...
                                <div className="flex-1">
                                    <span className="font-black block mb-1">Engine Error</span>
                        ...
                        <Button
                            ...
                            className={cn(
                                ...
                                error 
                                    ...
                                    : "bg-studio-accent hover:opacity-90 shadow-lg shadow-studio-accent/20"
                            )}
                        >
                            {isGeneratingScript ? (
